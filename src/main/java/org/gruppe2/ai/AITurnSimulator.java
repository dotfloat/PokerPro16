...
public class AITurnSimulator {
	...
	/**
	 * Simulates a number of rounds with the current AI hand. Returns a hand strength
	 * which can be used to decide which actions to perform
	 * @param roundPlayer
	 * @param communityCards
	 * @param numberOfRounds number of rounds to simulate
     * @return
     */
	public double getHandStregth(RoundPlayer roundPlayer, List<Card> communityCards, int numberOfRounds, int numOtherPlayers){
		...
		for (int j = 0;j<numOtherPlayers;j++){
			RoundPlayer rp = new RoundPlayer(null,this.drawCard(deck),this.drawCard(deck));
			roundPlayers.add(rp);
		}
		roundPlayers.add(roundPlayer);
		List<RoundPlayer> winners = sd.getWinnersOfRound(roundPlayers, communityCopy);
